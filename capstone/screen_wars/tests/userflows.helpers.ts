...
export async function challengeRun___shrinathSlashedSomeDays__bergMaxStreak(svm: LiteSVM, programId: PublicKey, shrinath: Keypair, berg: Keypair, challengeId: number) {
    ...
    await methods.syncAndLock(svm, programId, challengeId, berg, {
        logTxResult: false, debug: {
            user_passed: true,
            days_not_synced: 0,
            synced_today: false,
...
export async function secondChallengeCreatedByBerg____JoinedAndLockedByApaar(svm: LiteSVM, programId: PublicKey, berg: Keypair, apaar: Keypair, challengeId: number) {
    ...
    await methods.createChallenge(svm, programId, startTime, dailyTimer, berg, globalPDA, challengePDA, { logTxResult: false });

    // apaar joins challenge_id : 2
    let [apaarUserPDA] = await state.getUserPDAAddressAndBump(apaar.publicKey, programId);
    await methods.joinChallenge(svm, programId, globalPDADataBefore.challenge_ids, apaar, apaarUserPDA, globalPDA, challengePDA, { logTxResult: false });

    // teleport to challenge start
    let secondChallengeStart = Number((await state.fetchChallengePda(svm, programId, challengeId)).start);
    await clock.setTimeStamp(svm, secondChallengeStart);

    // apaar passes and syncs on first day
    await methods.syncAndLock(svm, programId, challengeId, apaar, {
        logTxResult: false,
        debug: {
            user_passed: true,
            days_not_synced: 0,
