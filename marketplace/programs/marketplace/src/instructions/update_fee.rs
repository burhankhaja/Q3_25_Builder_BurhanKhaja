...
impl<'info> UpdateFee<'info> {
    pub fn update(&mut self, new_fee: u16) -> Result<()> {
        ...
        require!(now.checked_sub(self.global.new_fee_at).unwrap() > one_week_in_seconds, MarketplaceErrors::FeeUpdateDelay);
        ...
        msg!("Fee change from {:?} to {:?} after 2 week period initiated", self.global.fee, new_fee);
        Ok(())
    }
}
